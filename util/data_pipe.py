...
class data_pipe():
    ...
    def make_pipe(self, batch_size, tensor_tuple, pretreatment=None):
        ...
        :return:
        '''
        data_set = tf.data.Dataset.from_tensor_slices(tensor_tuple)
        if pretreatment is not None:
            data_set = data_set.map(pretreatment, num_parallel_calls=self._num_parallel_calls)
        data_set = data_set.shuffle(buffer_size=1000).batch(batch_size, drop_remainder=True).repeat(count=None)
        data_set = data_set.prefetch(batch_size * self._num_parallel_calls)
        next_step = data_set.make_one_shot_iterator().get_next()
        return next_step
