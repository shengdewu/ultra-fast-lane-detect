    ...
    def regularization(self, scale):
        ...
        for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES):
            if v.name.find('weights') != -1:
                reg_weight.append(v)
        return tf.contrib.layers.apply_regularization(regularizer, reg_weight)
            ...
            with tf.Session() as sess:
                ...
                for step in range(pipe['total_epoch']):
                    if step % pipe['img_epoch'] == 0:
                        epoch += 1
                    _, total_loss, reg_loss, p, gs, lr, train_summary = sess.run([train_op, pipe['total_loss'], pipe['reg_loss'], precision, global_step, learning_rate, train_summary_op])
